...
export async function POST(request: Request) {
  ...
  try {
    ...
    const updated = await query(`UPDATE "Orders" SET payment_status = 'PAID', order_status = 'PROCESSING', paid_at = NOW(), provider_payment_id = $1, provider_signature = $2 WHERE id = $3 AND payment_status <> 'PAID' RETURNING id`, [input.paymentId, input.signature, input.orderId]);
    if (!updated.rowCount) return Response.json({ ok: true, redirectUrl: process.env.CUSTOMER_APP_URL ?? "https://app.website.com" });
    await query(`UPDATE "Payments" SET status = 'PAID', provider_payment_id = $1 WHERE order_id = $2`, [input.paymentId, input.orderId]);
    await logActivity({ actorType: "CUSTOMER", actorId: session.userId, action: "payment_success", targetType: "Orders", targetId: input.orderId, metadata: { paymentId: input.paymentId } });
    const allocation = await allocateOrder(input.orderId, session.userId);
    return Response.json({ ok: true, allocation, redirectUrl: process.env.CUSTOMER_APP_URL ?? "https://app.website.com" });
  } catch (error) {
